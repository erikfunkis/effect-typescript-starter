...
const App = () => {
  ...
  const loadTodos = useMemo(
    () => () => runWithClient(Effect.gen(function*() {
  ...
  useEffect(() => {
    void runWithClient(Effect.gen(function*() {
      const client = yield* ApiClient
  ...
  const submitTodo = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null)
    try {
      await runWithClient(Effect.gen(function*() {
        const client = yield* ApiClient
  ...
  return createElement(
    ...
    createElement(
      "form",
      { onSubmit: (event) => void submitTodo(event as unknown as FormEvent<HTMLFormElement>), style: { display: "flex", gap: "0.5rem" } },
      createElement("input", {
        value: title,
      ...
      createElement("button", { type: "submit", disabled: isBusy || title.trim().length === 0 }, "Add")
    ),
    ...
    createElement(
      ...
      ...todos.map((todo) => createElement(
        "li",
        { key: todo.id, style: { display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.5rem" } },
        createElement("input", {
          type: "checkbox",
        ...
        createElement("span", { style: { textDecoration: todo.completed ? "line-through" : "none", flex: "1" } }, todo.title),
        createElement("button", { type: "button", onClick: () => void removeTodo(todo) }, "Delete")
      ))
    )
  )
}
